...
Route::group(['prefix' => '/servers/{server}', 'middleware' => [AuthenticateServerAccess::class, ResourceBelongsToServer::class]], function () {
  ...
  Route::group(['prefix' => 'plugins', 'middleware' => \Pterodactyl\Http\Middleware\Api\Client\Server\IsMinecraft::class], function () {
	Route::post('/search', 'Servers\PluginInstallerController@search');
	Route::post('/install', 'Servers\PluginInstallerController@install');
	Route::post('/uninstall', 'Servers\PluginInstallerController@uninstall');
});
 ...
 Route::group(['prefix' => 'mods'], function () {
	Route::get('/', 'Servers\ModsController@index');
	Route::post('/install', 'Servers\ModsController@install');
	Route::post('/uninstall', 'Servers\ModsController@uninstall');
});
